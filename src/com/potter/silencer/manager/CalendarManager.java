...
//		Uri uri = Uri.parse("content://com.android.calendar/instances");
	...
	public PendingIntent prepareStartIntent(final CalendarEventInstance instance) {
		Intent startIntent = new Intent(ACTION_START_SILENCE, null, mContext,
				SilencerBroadcastReceiver.class);
		startIntent.putExtra(EXTRA_INSTANCE_ID, instance.getId());
		PendingIntent startAlarmIntent = PendingIntent.getBroadcast(mContext,
				0, startIntent, 0);
		return startAlarmIntent;
	}

	public PendingIntent prepareEndIntent(final CalendarEventInstance instance) {
		Intent endIntent = new Intent(ACTION_END_SILENCE, null, mContext,
				SilencerBroadcastReceiver.class);
		endIntent.putExtra(EXTRA_INSTANCE_ID, instance.getId());
		PendingIntent endAlarmIntent = PendingIntent.getBroadcast(mContext, 0,
				endIntent, 0);
	...
	public CalendarManager createAlarm(final CalendarEventInstance instance) {
		...
		getAlarmManager().set(AlarmManager.RTC_WAKEUP, instance.getBegin(),
				prepareStartIntent(instance));
		getAlarmManager().set(AlarmManager.RTC_WAKEUP, instance.getEnd(),
				prepareEndIntent(instance));
		return this;
	}
